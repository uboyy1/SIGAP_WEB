...
// frontend/src/pages/kepalaTeknisi/EditProfilKepalaTeknisi.jsx
import { useState, useEffect, useRef, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import { deleteMyProfilePicture, getCurrentUser, updateProfile, updatePassword, uploadMyProfilePicture } from '../../services/api';
...
export default function EditProfilKepalaTeknisi() {
  ...
  const refreshCurrentUser = useCallback(async () => {
    ...
    if (refreshedUser && isMounted.current) {
      setUser(refreshedUser);
      return refreshedUser;
    }

    const userRes = await getCurrentUser();
    if (userRes.success && userRes.data && isMounted.current) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (isMounted.current) navigate('/kepala-teknisi/profil');
      }, 1500);
    } catch (err) {
  ...
  if (!user) {
    return <div className="flex justify-center items-center h-64 text-gray-500">Memuat data profil...</div>;
  }

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="bg-gradient-to-r from-primary-600 to-primary-800 rounded-xl shadow-lg overflow-hidden">
        <div className="p-6 flex items-center gap-4">
          ...
          <div>
            <h2 className="text-white font-bold text-xl">Edit Profil Kepala Teknisi</h2>
            <p className="text-white/75 text-sm">Perbarui informasi profil kepala teknisi</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-card overflow-hidden">
        <form onSubmit={handleSubmit} className="p-6" noValidate>
          ...
          <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-gray-200 dark:border-gray-700">
            <button
              type="button"
              onClick={() => navigate('/kepala-teknisi/profil')}
